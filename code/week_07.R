...
t.test(xs,ys, var.equal = F)
t.test(xl,yl, var.equal = F)
...
fakety_test <- function(x,y) {
  ...
  Tval <- (MUx-MUy)/(sqrt((SDx^2/Nx)+(SDy^2/Ny)))
  print(Tval)
  
  df <- ((SDx^2/Nx)+(SDy^2/Ny)) / (((SDx^2/Nx)^2)/(Nx-1))+(((SDy^2/Ny)^2)/(Ny-1))
  print(df)
}

fakety_test(x=a1,y=b2)
t.test(a1,b2)
